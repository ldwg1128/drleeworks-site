...
<!-- 추후 그림 추가: 일반적인 인장시험 구성과 stress–strain curve -->
...
| 구성 요소 | 역할 |
| --- | --- |
| 하중계(Load cell) | 시편에 전달되는 인장력을 전기 신호로 변환하여 측정한다. |
| 크로스헤드(Crosshead) | 구동 장치에 의해 이동하며 시편에 변위를 가한다. |
| 그립 또는 지그(Grip or fixture) | 시편 양단을 고정하고 하중을 전달한다. |
| 시편(Specimen) | 규격에 맞게 제작된 시험 대상이다. |
| 신률계(Extensometer) | 정해진 표점 구간에서 시편의 실제 연신량을 측정한다. |
| 데이터 수집 시스템 | 하중, 위치, 연신량 등의 신호를 시간에 따라 동기화하여 저장한다. |
