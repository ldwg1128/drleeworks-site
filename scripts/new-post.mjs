...
const categories = {
  ko: { computing: '컴퓨팅', engineering: '공학', notes: '노트', recommendations: '추천' },
...
if (!categories[lang]?.[section] || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug ?? '') || !title) {
  console.error('사용법: npm run post:new -- <ko|en> <computing|engineering|notes|recommendations> <slug> <제목>');
  process.exit(1);
}
