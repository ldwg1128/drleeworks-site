...
const categories = {
  ko: { computing: '컴퓨팅', engineering: '공학', notes: '기록' },
  en: { computing: 'Computing', engineering: 'Engineering', notes: 'Notes' }
};
if (!categories[lang]?.[section] || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug ?? '') || !title) {
  console.error('사용법: npm run post:new -- <ko|en> <computing|engineering|notes> <slug> <제목>');
  process.exit(1);
}
const directory = path.resolve('src/content/posts', lang, section);
